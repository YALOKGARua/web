import { DOMElements, LanguageCode } from './types.js';
...
class BNBSolution {
    ...
    private currentReviewIndex: number;
    ...
    constructor() {
        this.elements = {
            ...
            canvas: document.getElementById('_z9q8w7') as HTMLCanvasElement | null,
            progressBar: createProgressBar()
        };

        this.currentLang = (localStorage.getItem('_l9k0') as LanguageCode) || 'uk';
    ...
    private initReviewSlider(): void {
        if (!this.elements.reviewsContainer || !this.elements.reviewsPath || !this.elements.reviewNodes.length) return;
    ...
    private initModals(): void {
        ...
        Object.entries(modals).forEach(([modalId, triggerClass]) => {
            ...
            const setModalDisplay = (display: 'block' | 'none') => {
    ...
    private handleContactForm(event: Event): void {
        ...
        const nameInput = form.querySelector(`#${form.id}-name`) as HTMLInputElement;
        const phoneInput = form.querySelector(`#${form.id}-phone`) as HTMLInputElement;
        const msgElement = form.querySelector(`#${form.id}-msg`) as HTMLElement;
        ...
        setTimeout(() => {
            const modal = form.closest('._m0n9b8') as HTMLElement;
            if (modal) modal.style.display = 'none';
        }, 1500);
    }

    private handleJobForm(event: Event): void {
        event.preventDefault();
        const form = event.target as HTMLFormElement;
        const nameInput = form.querySelector(`#${form.id}-name`) as HTMLInputElement;
        const phoneInput = form.querySelector(`#${form.id}-phone`) as HTMLInputElement;
        const roleSelect = form.querySelector('#_j4k3l2') as HTMLSelectElement;
        const msgElement = form.querySelector(`#${form.id}-msg`) as HTMLElement;
    ...
    private initScrollHandling(): void {
        const handleScroll = () => {
    ...
    private handleSectionVisibility(): void {
    ...
    private updateVisitCounter(): void {
